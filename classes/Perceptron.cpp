...
Perceptron &Perceptron::operator=(const Perceptron &other) {
	if (this != &other) {
		...
		for (size_t i = 0; i < len; i++) this->weight[i] = other.weight[i];
	}
	return (*this);
}

float Perceptron::feedFoward(const Vector2 *inputArray,
							 const size_t	len) const {
	Vector2 vecsum = Vector2Zero();
	for (size_t i = 0; i < len; i++) vecsum += inputArray[i] * this->weight[i];
...
void Perceptron::train(const Vector2 &input, const size_t index,
					   const float desired) {
	const size_t len = 1;
...
float Perceptron::getWeightedX0(const float	 x0,
								const size_t weightIndex) const {
	if (weightIndex >= this->weightLen)
		return (x0 * this->weight[this->weightLen - 1].x);
	return (x0 * this->weight[weightIndex].x);
}

float Perceptron::getWeightedX1(const float	 x1,
								const size_t weightIndex) const {
	if (weightIndex >= this->weightLen)
...
const Vector2 &Perceptron::getWeightAt(const size_t index) const {
	if (index >= this->weightLen) return (this->weight[this->weightLen - 1]);
